...
def finite_difference_method(p, q, a, b, n,
    ...
    for k in range(n + 1):
        lower[k] = 1 - step * p / 2
        main[k] = -2 + step * step * q
        upper[k] = 1 + step * p / 2
    upper[0] = 2 * beta0 / step
    main[0] = alpha0 - 3 * beta0 / (2 * step)
    main[n] = alpha1 + 3 * beta1 / (2 * step)
    lower[n] = -2 * beta1 / step
    k_0 = -beta0 / (2 * step)
    k_1 = beta1 / (2 * step)

    upper[0] = upper[0] * upper[1] - main[1] * k_0
    main[0] = main[0] * upper[1] - lower[1] * k_0
    main[n] = main[n] * lower[n - 1] - upper[n - 1] * k_1
    lower[n] = lower[n] * lower[n - 1] - main[n - 1] * k_1
    diff_op[0] = diff_op[0] * upper[1] - diff_op[1] * k_0
    diff_op[n] = diff_op[n] * lower[n - 1] - diff_op[n - 1] * k_1
